...
export class CoreWebrtcService {
    ...
    return new Promise<boolean>((resolve, reject) => {
      try {
        ...
        if (connectionStatus === AppConstants.CONNECTION_STATES.NOT_CONNECTED) {
          ...
          webrtcContext[AppConstants.CONNECTION] = new RTCPeerConnection(
            AppConstants.STUN_CONFIG
          );
  ...
  async handleAnswer(signalingMessage: any): Promise<void> {
    ...
    peerConnection.setRemoteDescription(
      new RTCSessionDescription(signalingMessage.answer)
    );
    return;
  }
  ...
  async handleCandidate(signalingMessage: any): Promise<void> {
    ...
    peerConnection.addIceCandidate(
      new RTCIceCandidate(signalingMessage.candidate)
    );
    return;
  }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (let i = 0; i < requiredMediaTracks.length; i++) {
           ...
           */
          const stream: any = await this.coreMediaCaptureService.getMediaStream(
            requiredMediaTracks[i]
          );
  ...
  public getDataChannelOffer(peerConnection: any, mediaChannel: string) {
    return new Promise(async (resolve, reject) => {
      const dataChannel = await this.openDataChannel(
        peerConnection,
        mediaChannel
      );
      peerConnection
    ...
    return new Promise((resolve, reject) => {
      ...
      peerConnection
        ...
        .then(async (newAnswer: any) => {
           ...
           */
          const maxBitrate =
            this.coreAppUtilService.getMaxBitrateForSdpModification(
              mediaChannel
            );
          const sdpMediaType =
            this.coreAppUtilService.getMediaTypeForSdpModification(
              mediaChannel
            );
          const modifiedSdp: any = await this.setMediaBitrate(
            newAnswer.sdp,
            sdpMediaType,
            maxBitrate
          );
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (let i = 0; i < requiredMediaTracks.length; i++) {
        ...
        switch (requiredMediaTracks[i]) {
          case AppConstants.AUDIO:
            streamContext[AppConstants.TRACK] = stream.getAudioTracks()[0];
            streamContext[AppConstants.TRACK_SENDER] = peerConnection.addTrack(
      ...
      peerConnection
        ...
        .then(async (newAnswer: any) => {
           ...
           */
          const maxBitrate =
            this.coreAppUtilService.getMaxBitrateForSdpModification(channel);
          const sdpMediaType =
            this.coreAppUtilService.getMediaTypeForSdpModification(channel);
          const modifiedSdp: any = await this.setMediaBitrate(
            newAnswer.sdp,
            sdpMediaType,
            maxBitrate
