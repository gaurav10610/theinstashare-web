...
export class TalkWindowWebrtcService {
  ...
  setupJobToCleanIdleDataConnection(channel: string, username: string) {
    ...
    const recurringJobId: any = setInterval(() => {
      LoggerUtil.logAny(
        "running recurring job to clean up idle " +
          channel +
          " connection for user: " +
          username
      );
      const dataChannel: any = this.coreAppUtilService.getNestedValue(
        userWebrtcContext,
      ...
      if (
        ...
        const lastUsedTime: any =
          userWebrtcContext[AppConstants.MEDIA_CONTEXT][channel][
        ...
        if (idleTime > AppConstants.DATACHANNEL_IDLE_THRESHOLD) {
          LoggerUtil.logAny(
            channel +
              " data channel with user " +
              username +
              " was idle so cleaning it up"
          );
        ...
        const recurringJobId: any = this.coreAppUtilService.getNestedValue(
          userWebrtcContext,
          AppConstants.MEDIA_CONTEXT,
          channel,
          AppConstants.RECURRING_JOB_ID
        );
        if (recurringJobId) {
          LoggerUtil.logAny(
            "removed recurring job for " +
              channel +
              " connection with user " +
              username
          );
    ...
    LoggerUtil.logAny(
      "a recurring job has been setup to clean up idle " +
        channel +
        " connection for user " +
        username
    );
  ...
  registerWebrtcEventListeners(peerConnection: any, userToChat: any) {
    return new Promise<void>((resolve, reject) => {
      LoggerUtil.logAny(
        "registering webrtc events on webrtc connection for " + userToChat
      );
      try {
        /**
         *
        ...
        peerConnection.onnegotiationneeded = async (event) => {
          LoggerUtil.logAny(
            userToChat + " webrtc connection needs renegotiation"
          );
        };
        ...
        peerConnection.onsignalingstatechange = () => {
          this.zoneRef.run(() => {
            LoggerUtil.logAny(
              userToChat +
                " webrtc connection signaling state: " +
              ...
              case "stable":
                ...
                while (
                  ...
                  const callback: CallbackContextType = <CallbackContextType>(
                    webrtcContext[
                      AppConstants.WEBRTC_ON_CONNECT_QUEUE
                    ].dequeue()
                  );
                  try {
        ...
        LoggerUtil.logAny(
          "there is an error while registering events on peer connection"
        );
        reject(error);
      }
    });
      ...
      this.zoneRef.run(() => {
        ...
        switch (channel) {
          case AppConstants.AUDIO:
            ...
            setTimeout(() => {
              this.talkWindowSetRemoteVolumeFn(1.0);
            }, 2000);
         ...
         *
         */
        this.talkWindowOnMediaStreamReceivedFn(
        ...
        if (channel === AppConstants.SCREEN || AppConstants.SOUND) {
          ...
          if (
            this.talkWindowContextService.bindingFlags.haveLocalAudioStream &&
          ...
          //channel === 'video'
          if (
            this.talkWindowContextService.bindingFlags.haveLocalVideoStream &&
    ...
    switch (signalingMessage.request) {
      case AppConstants.INVITE:
        ...
        if (
           ...
           */
          this.talkWindowContextService.remoteAccessContext["canAccessRemote"] =
            signalingMessage.isNativeApp;
  ...
  cleanWebrtcContext() {
    return new Promise<void>((resolve, reject) => {
      try {
         ...
         */
        Object.keys(this.userContextService.webrtcContext).forEach(
  ...
  cleanMediaStreamContext(channel: string, mediaChannelContext: any) {
    return new Promise<void>((resolve, reject) => {
      try {
        ...
        LoggerUtil.logAny(
          "there is an error occured while cleaning media channel context for channel: " +
            channel
        );
        reject(error);
      }
  ...
  cleanDataChannelContext(channel: string, mediaChannelContext: any) {
    return new Promise<void>((resolve, reject) => {
      try {
          ...
          if (channel === AppConstants.REMOTE_CONTROL) {
            ...
            if (this.talkWindowContextService.bindingFlags.isAccessingRemote) {
              this.talkWindowContextService.bindingFlags.isAccessingRemote =
                false;
              this.appUtilService.removeRemoteAccessEventListeners(
        ...
        LoggerUtil.logAny(
          "there is an error occured while cleaning data channel context for channel: " +
            channel
        );
        reject(error);
      }
  ...
  cleanChannelContextIfNotConnected(username: string, channel: string) {
    ...
    webrtcContext[AppConstants.MEDIA_CONTEXT][channel][
      ...
      const connected: boolean = this.coreAppUtilService.isDataChannelConnected(
        webrtcContext,
        channel
      );
      if (!connected) {
        ...
        const popupContext: any = this.messageService.buildPopupContext(
          AppConstants.POPUP_TYPE.UNABLE_TO_CONNECT,
          channel
        );
        this.talkWindowFlagPopupMessageFn(
          popupContext,
        ...
        LoggerUtil.logAny(
          channel +
            " data channel connection with " +
            username +
            " found connected after timeout"
        );
      }
      ...
      if (!connected) {
        ...
        if (mediaContext[channel][AppConstants.TRACK_SENDER]) {
          webrtcContext[AppConstants.CONNECTION].removeTrack(
            mediaContext[channel][AppConstants.TRACK_SENDER]
          );
        }
        ...
        LoggerUtil.logAny(
          "media stream connection for " +
            channel +
            " channel found connected after timeout"
        );
      }
    }, AppConstants.CONNECTION_TIMEOUT);
  }
   ...
   */
  processChannelStreamDisconnect(
    channel: string,
    username: string,
    sendDisonnectNotification: boolean,
    popupContexts?: any[]
  ) {
    LoggerUtil.logAny(
      "stopping " + channel + " stream session with " + username
    );
    ...
    if (popupContexts) {
      popupContexts.forEach((popupContext: any) => {
        ...
        setTimeout(() => {
          this.talkWindowRemovePopupContextFn([popupContext.type]);
        }, AppConstants.CALL_DISCONNECT_POPUP_TIMEOUT);
      });
    }
  }
  ...
  webrtcConnectionDisconnectHandler(username: string, popupContexts?: any[]) {
    return new Promise<void>((resolve) => {
      LoggerUtil.logAny(
        "handling webrtc connection disconnect for " + username
      );
      const webrtcContext: any =
        this.userContextService.getUserWebrtcContext(username);
      ...
      switch (message.type) {
        ...
        default:
          ...
          const messageStatus: string =
            await this.talkWindowUpdateChatMessagesFn(message);
          if (
            messageStatus !== AppConstants.CHAT_MESSAGE_STATUS.NOT_APPLICABLE
          ) {
            this.appUtilService.sendMessageAcknowledgement(
              message,
              messageStatus,
              message.type
            );
  ...
  async processFileMessage(message: any) {
    ...
    switch (message.chunkType) {
      ...
      case AppConstants.CHUNK_TYPE.WHOLE:
        if (
          message[AppConstants.USERNAME] === this.userContextService.userToChat
        ) {
        ...
        const id =
          this.talkWindowContextService.contentIdsMap[
            message[AppConstants.CONTENT_ID]
          ];
        delete this.talkWindowContextService.contentIdsMap[
          message[AppConstants.CONTENT_ID]
        ];
      ...
      case AppConstants.CHUNK_TYPE.INTERMEDIATE:
        ...
        buffer =
          webrtcContext[AppConstants.MEDIA_CONTEXT][AppConstants.FILE][
            message[AppConstants.CONTENT_TYPE]
          ];
      ...
      case AppConstants.CHUNK_TYPE.END:
         ...
         */
        this.talkWindowContextService.sharedContent[newContentId] =
          buffer.join("");
      ...
      switch (eventObject.property) {
        case "haveLocalAudioStream":
          if (eventObject.channel === AppConstants.SOUND) {
            this.talkWindowContextService.bindingFlags.isSoundSharing =
              propertyValue;
            this.talkWindowContextService.bindingFlags.isAudioSharing =
              propertyValue;
          }
          break;
        ...
        case "haveLocalVideoStream":
          if (eventObject.channel === AppConstants.SCREEN) {
            this.talkWindowContextService.bindingFlags.isScreenSharing =
              propertyValue;
            this.talkWindowContextService.bindingFlags.isVideoSharing =
              propertyValue;
          }
          //close the full screen mode and hide modal popup
          if (
            !this.userContextService.isMobile &&
  ...
  setUpWebrtcConnection(username: string, offerMessage?: any) {
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        LoggerUtil.logAny(e);
        reject(
          "there is an exception occured while establishing connection with " +
            username
        );
      }
    });
  ...
  setUpDataChannel(createDataChannelType: CreateDataChannelType) {
    return new Promise<void>(async (resolve, reject) => {
      try {
         ...
         */
        if (
          !this.userContextService.hasUserWebrtcContext(
        ...
        if (
          ...
          LoggerUtil.logAny(
            "registered message listener on " +
              createDataChannelType.channel +
              " data channel"
          );
          ...
          webrtcContext[AppConstants.MEDIA_CONTEXT][
            createDataChannelType.channel
          ][AppConstants.DATACHANNEL] = dataChannel;
        } else {
          LoggerUtil.logAny(
            "webrtc connection is not in connected state for user: " +
              createDataChannelType.username
          );
          /**
           *
