...
import { CoreMediaCaptureService } from "src/app/services/media-capture/core-media-capture.service";
...
export class LoginComponent implements OnInit {
  constructor(
    ...
    this.matIconRegistry.addSvgIcon(
      ...
      this.domSanitizer.bypassSecurityTrustResourceUrl(
        this.assetsPath + "images/icons/github-icon.svg"
      )
    );
    this.matIconRegistry.addSvgIcon(
      "instashare_title",
      this.domSanitizer.bypassSecurityTrustResourceUrl(
        this.assetsPath + "images/icons/instashare-title.svg"
      )
    );
  }
  ...
  async ngOnInit() {
    ...
    try {
      const message: string =
        await this.coreMediaCaptureService.takeCameraAndMicrophoneAccess();
  ...
  async register() {
    ...
    const isUsernameTaken: Boolean =
      await this.signalingService.checkIfUsernameTaken(username);
    if (isUsernameTaken) {
      this.errorMessage = "username is either invalid or already been taken";
      this.isRegistering = false;
    } else {
    ...
    switch (message.type) {
      case AppConstants.REGISTER:
        ...
        if (message.success) {
          ...
          this.errorMessage =
            "username is either invalid or already been taken";
          this.isRegistering = false;
        }
        break;
      default:
